...
public class CommandActivity extends HhBaseActivity implements INaviInfoCallback {
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        for (int i = 0; i < oneBodyJsonList.size(); i++) {
                            if (oneBodyJsonList.get(i).getDto().getIsReal()!=null) {
                                if (oneBodyJsonList.get(i).getDto().getIsReal() == 1){
                        ...
                        for (int j = 0; j < oneBodyFireAllList.size(); j++) {
                            if (oneBodyFireAllList.get(j).getDto().getIsReal()==null) {
                                oneBodyFireList.add(oneBodyFireAllList.get(j));
                            }
                        }
                        if(oneBodyFireList.size()!=0){
                            flyBaiduMap84(oneBodyFireList.get(0).getDto().getAlarmLatitude(),oneBodyFireList.get(0).getDto().getAlarmLongitude());
                            initOneBodyFireMarker();
                        }else{
    ...
    private void initOneBodyFireMarker() {
        for (int i = 0; i < oneBodyFireList.size(); i++) {
            double[] doubles = LatLngChangeNew.calWGS84toBD09(oneBodyFireList.get(i).getDto().getAlarmLatitude(), oneBodyFireList.get(i).getDto().getAlarmLongitude());
            LatLng point = new LatLng(doubles[0], doubles[1]);
            Bundle bundle = new Bundle();
            bundle.putString("id", oneBodyFireList.get(i).getDto().getId());
