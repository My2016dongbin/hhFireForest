...
public class TrackService extends Service {
   ...
   // public LocationClient mLocationClient = null;
    ...
    private void changeUserPosition(User user) {
        Log.e(TAG, "changeUserPosition: CommonData.upload " + CommonData.upload );
        //设置
        if(!CommonData.upload){
            return;
        }
        //getInfo返回的指定签到时间段
        Date dateStart = new Date();
        dateStart.setHours(CommonData.earlyHour);
        dateStart.setMinutes(CommonData.earlyMinute);
        Date dateEnd = new Date();
        dateEnd.setHours(CommonData.nightHour);
        dateEnd.setMinutes(CommonData.nightMinute);
        Date now = new Date();
        Log.e(TAG, "changeUserPosition: " + dateStart + "," + dateEnd );
        if(now.before(dateStart) || now.after(dateEnd)){
            return;
        }
        Log.e(TAG, "changeUserPosition: " + dateStart + "," + dateEnd );
        Calendar calendar = Calendar.getInstance();
        String weekNum = calendar.get(Calendar.DAY_OF_WEEK)-1+"";
        Log.e(TAG, "changeUserPosition: " + CommonData.workDayList );
        Log.e(TAG, "changeUserPosition: " + weekNum );
        if((!CommonData.workDayList.contains(weekNum))){
            return;
        }
        Log.e(TAG, "changeUserPosition: -" );
