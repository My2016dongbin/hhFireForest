...
function onload(Cesium) {
    ...
    const baseLayer = imageryLayers.get(1)
        ...
        var subdomains = ['0', '1', '2', '3', '4', '5', '6', '7']
          const tiandituUrl = 'http://t{s}.tianditu.com/DataServer?T=cia_w&x={x}&y={y}&l={z}&tk=bddf1a22ff397dc7d25b9b34e4868da7'
...
//清除周边圆圈
function clearFireCircle() {
  const dataSource_ = viewer.dataSources.getByName("周边")
  if (dataSource_?.length >= 1) {
    dataSource_.forEach(item => {
...
//添加周边圆圈
function addFireCircle(config) {
  const defaultConfig = {
    lng: 120,
    lat: 36,
    dsName: 'FIRE_CIRCLE_DATA_SOURCE_NAME'
  }

  var configBf=JSON.parse(config)
  var configAf=JSON.parse(configBf)
  const { lng, lat, dsName } = { ...defaultConfig, ...configAf }

  const dataSource_ = viewer.dataSources.getByName(dsName)
  if (dataSource_?.length >= 1) {
    dataSource_.forEach(item => {
      viewer.dataSources.remove(item)
    })
  ...
  const dataSource = new Cesium.CustomDataSource(dsName)
  ...
  _drawCircle(dataSource, position, 1000.0, Cesium.Color.fromCssColorString('#C00018'))
  _drawCircle(dataSource, position, 3000.0, Cesium.Color.fromCssColorString('#FFA613'))
  _drawCircle(dataSource, position, 5000.0, Cesium.Color.fromCssColorString('#F8E71C'))
  _drawCircle(dataSource, position, 10000.0, Cesium.Color.fromCssColorString('#7ED321'))
  _drawCircle(dataSource, position, 15000.0, Cesium.Color.fromCssColorString('#4A90E2'))

  viewer.dataSources.add(dataSource)
}
function _drawCircle(dataSource, position, r, color) {
  dataSource.entities.add({
    position: position,
    ellipse: {
      height: 0.0,
...
dsBridge.register('addFireCircleDs', function (data) {
     console.log("onvalue");
     console.log(data);
   addFireCircle(data);
});

dsBridge.register('clearFireCircleDs', function (data) {
   clearFireCircle();
});
...
function flyToCenter(data) {
    const defaultCenter = {
        longitude: 118.340768237,
        latitude: 35.0724090744,
        height: 40000
    }
...
function flyTo(entities) {
    viewer.flyTo(entities, { duration: 2, offset: new Cesium.HeadingPitchRange(0, Cesium.Math.toRadians(-85), 2500) })
}
//andoird 请求显示谷歌在线
...
dsBridge.register('GPSflyto', function (gpsinfo) {
    ...
    if(gpsInfo1.position.lng!=undefined) {
        ...
        console.log("没打开GPS！");
        viewer.camera.flyTo({
            destination: Cesium.Cartesian3.fromDegrees(118.340768237,35.0724090744,15000.0)
        });
    }
});
