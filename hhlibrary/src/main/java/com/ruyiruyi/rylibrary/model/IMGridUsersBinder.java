...
public class IMGridUsersBinder extends ItemViewProvider<IMGridUsers, IMGridUsersBinder.ViewHolder> {
    ...
    @Override
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull final IMGridUsers imGridUsers) {
        ...
        RxViewAction.clickNoDouble(holder.iv_status).subscribe(unused -> {
            ...
            listener.OnBeiDouGridSelect(imGridUsers.getUsers(),imGridUsers.isStatus());
        });
        if(imGridUsers.isStatus()){
            holder.iv_status.setImageDrawable(context.getResources().getDrawable(R.mipmap.ic_se));
        }else{
    ...
    private void drawUsers(ViewHolder holder, IMGridUsers imGridUsers,boolean auto) {
        if(imGridUsers.getUsers()!=null){
            holder.ll_users.removeAllViews();
            for (int i = 0; i < imGridUsers.getUsers().size(); i++) {
                IMGridUsers.Users users = imGridUsers.getUsers().get(i);
    ...
    public interface OnBeiDouItemClickListener{
        void OnBeiDouItemClick(IMGridUsers imGridUsers);
        void OnBeiDouGridSelect(List<IMGridUsers.Users> list, boolean status);
        void OnBeiDouUsersSelect(IMGridUsers.Users users,boolean status);
    }
}
