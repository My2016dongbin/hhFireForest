...
public class IMActivity extends HhBaseActivity implements IMGridUsersBinder.OnBeiDouItemClickListener {
    ...
    private void postData() {
        ...
        //params.addParameter("gridNo","370900");
        params.addParameter("gridNo",new DbConfig(this).getUser().getGridNo());
        Log.e(TAG, "postData: gridNo " + new DbConfig(this).getUser().getGridNo() );
                    ...
                    if(object.getInt("code")==200){
                        ...
                        if(data!=null && data.length()!=0){
                            ...
                            if(c.getString("name").contains("街道")){
                                Log.e(TAG, "onSuccess: jiedao" );
                                Gson gson = new Gson();
                                dataList = gson.fromJson(String.valueOf(data), new TypeToken<List<IMGridUsers>>() {}.getType());

                                if(Objects.equals(new DbConfig(IMActivity.this).getUser().getGridNo(), "370900")){
                                    IMGridUsers gridUsers = new IMGridUsers();
                                    gridUsers.setName("泰安市");
                                    gridUsers.setNo("370900");
                                    dataList.add(gridUsers);
                                }
                                for (int m = 0; m < dataList.size(); m++) {
                                    IMGridUsers imGridUsers = dataList.get(m);
                                    List<IMGridUsers> imGridUsersChildren = imGridUsers.getChildren();
                                    dataList.addAll(imGridUsersChildren);
                                }
                            ...
                            //邹平市权限
                            else if(Objects.equals(c.getString("name"), "泰安市")){
                                Log.e(TAG, "onSuccess: taian" );
                                Gson gson = new Gson();
                                dataList = gson.fromJson(String.valueOf(c.getJSONArray("children")), new TypeToken<List<IMGridUsers>>() {}.getType());
                                List<IMGridUsers> list_ = new ArrayList();

                                /*for (int m = 0; m < dataList.size(); m++) {
                                    IMGridUsers imGridUsers = dataList.get(m);
                                    list_.add(imGridUsers);
                                    try{
                                        List<IMGridUsers> imGridUsersChildren = imGridUsers.getChildren();
                                        dataList.addAll(imGridUsersChildren);
                                        list_.addAll(imGridUsersChildren);
                                    }catch (Exception e){
                                        Log.e(TAG, "onSuccess: taian " + e );
                                    }
                                }
                                dataList = new ArrayList<>(list_);*/
                                if(Objects.equals(new DbConfig(IMActivity.this).getUser().getGridNo(), "370900")){
                                    IMGridUsers gridUsers = new IMGridUsers();
                                    gridUsers.setName("泰安市");
                                    gridUsers.setNo("370900");
                                ...
                                for (int m = 0; m < children.length(); m++) {
                                    ...
                                    if(Objects.equals(cell.getString("name"), "泰安市")){
                                        ...
                                        for (int n = 0; n < dataList.size(); n++) {
                                            IMGridUsers imGridUsers = dataList.get(n);
                                            List<IMGridUsers> imGridUsersChildren = imGridUsers.getChildren();
                                            dataList.addAll(imGridUsersChildren);
                                        }
    ...
    private void postUsers() {
        for (int i = 0; i < dataList.size(); i++) {
            ...
            Log.e(TAG, "postUsers: --" + params + object.toString());
            int finalI = i;
            x.http().post(params, new Callback.CommonCallback<String>() {
                @Override
                public void onSuccess(String result) {
                    Log.e(TAG, "postUsers: --" + result);
                    try {
                        JSONObject object = new JSONObject(result);
                        if(object.getInt("code")==200){
                            JSONArray data = object.getJSONArray("data");
                        ...
                        Log.e(TAG, "postData: onSuccess e2" + e );
                        e.printStackTrace();
                    }
                }
        ...
        RxViewAction.clickNoDouble(tv_go).subscribe(new Action1<Void>() {
            ...
            public void call(Void unused) {
                ...
                if(selectList.size() > 1){
                    Toast.makeText(IMActivity.this, "最多选择一名通话人员", Toast.LENGTH_SHORT).show();
                    return;
                }
                inviteType = "audio";
                //创建房间
    ...
    private void inviteOther() {
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            param.customInfo(new DbConfig(IMActivity.this).getUser().getFullName()/*+"@"+inviteType*/);//传递额外参数(发起人名字&&通话类型) 旧版
