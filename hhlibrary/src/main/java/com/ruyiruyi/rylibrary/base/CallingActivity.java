...
public class CallingActivity extends HhBaseActivity implements NERtcCallback {
    ...
    private NERtcVideoView video_view_my;
    private NERtcVideoView video_view_other;
    ...
    private boolean isCalling;//true 主动拨打
    private boolean isVideo;//true 视频通话
    private User user;
     ...
     * 本地视频影像参数与初始化
     */
    private void localVideoPreview() {
        if(!isVideo){
            return;
        }
        video_view_other.setVisibility(View.VISIBLE);
        //设置本地视频参数
        ...
        //设置本地画布
        NERtcEx.getInstance().setupLocalVideoCanvas(video_view_other);
    ...
    private boolean lookLocal = true;
    /**
     * 交换本地视频与对方视频现实位置
     */
    void changeVideoLocalOrOther(){
        lookLocal = !lookLocal;
        if(lookLocal){
            video_view_my.clearImage();
            video_view_other.clearImage();
            //交换画布
            NERtcEx.getInstance().setupLocalVideoCanvas(video_view_other);
            NERtcEx.getInstance().setupRemoteVideoCanvas(video_view_my,uid);
        }else{
            video_view_my.clearImage();
            video_view_other.clearImage();
            //交换画布
            NERtcEx.getInstance().setupLocalVideoCanvas(video_view_my);
            NERtcEx.getInstance().setupRemoteVideoCanvas(video_view_other,uid);
        }
    }
    ...
    private void bind_() {
        RxViewAction.clickNoDouble(video_view_my).subscribe(unused -> changeVideoLocalOrOther());
    ...
    private void init_() {
        ...
        video_view_my = findViewById(R.id.video_view_my);
        video_view_other = findViewById(R.id.video_view_other);
        video_view_my.setVisibility(View.GONE);
        video_view_other.setVisibility(View.GONE);
        //小画布置于大画布上面
        video_view_my.setZOrderMediaOverlay(true);
        ...
        isVideo = intent.getBooleanExtra("isVideo",false);//通话邀请参数需与前端统一规范
        if(!isCalling){
            //被邀请
            ...
            tv_time.setText(isVideo?"视频邀请":"语音邀请");
            ...
            if(CommonData.personList.size()>1){
                ...
                if(!CommonData.personList.isEmpty()){
                    IMOnlineUsers.Users user_ = CommonData.personList.get(0);
            ...
            uid = Long.parseLong(CommonData.inviteOtherId);//TODO 待优化
        }
                ...
                new RequestCallbackWrapper<ChannelFullInfo>() {
                    ...
                    public void onResult(int code, ChannelFullInfo channelFullInfo, Throwable throwable) {
                        ...
                        if (code == ResponseCode.RES_SUCCESS) {
                            ...
                            //主动-被邀请者接受邀请后主动订阅视频开始载入
                            NERtcEx.getInstance().setupRemoteVideoCanvas(video_view_other,Long.parseLong(accountId));
    ...
    public void onUserLeave(long l, int i) {
        ...
        if(!isCalling){
            ...
            for (int j = 0; j < CommonData.personList.size(); j++) {
                IMOnlineUsers.Users users_ = CommonData.personList.get(j);
...
/*    @Override
    ...
    public void onUserVideoStart(long l, int i) {
        ...
        //被动-载入视频影像(有人开始订阅视频影像时载入)
        NERtcEx.getInstance().setupRemoteVideoCanvas(video_view_other,uid);
        NERtcEx.getInstance().subscribeRemoteVideoStream(uid, NERtcRemoteVideoStreamType.kNERtcRemoteVideoStreamTypeHigh,true);
        changeVideoLocalOrOther();
        //显示本地视频影像
        video_view_my.setVisibility(View.VISIBLE);
        //头像隐藏&&计时下移
        ll_one.setVisibility(View.GONE);
        tv_time2.setVisibility(View.VISIBLE);
    }
