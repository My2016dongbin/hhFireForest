...
public class FireMissionInfoActivity extends HhBaseActivity implements INaviInfoCallback {
    ...
    private void initView() {
         ...
         * 导航选择Dialog
         **/
        guideChooseDialog = new Dialog(this, R.style.ActionSheetDialogStyle);
        guideChooseInflater = LayoutInflater.from(this).inflate(R.layout.dialog_guide_choose, null);
    ...
    private void openGaoDeMap() {
        double[] doubles = LatLngChangeNew.calWGS84toGCJ02(fireMission.getPosition().getLat(), fireMission.getPosition().getLng());
        if (MapUtil.isGdMapInstalled()) {
            MapUtil.openGaoDeNavi(this, 0, 0, null, doubles[0], doubles[1], fireMission.getTaskContent()+"地址");
        } else {
    ...
    private void openBaiDuMap() {
        double[] doubles = LatLngChangeNew.calWGS84toBD09(fireMission.getPosition().getLat(), fireMission.getPosition().getLng());
        if (MapUtil.isBaiduMapInstalled()) {
            MapUtil.openBaiDuNavi(this, 0, 0, null, doubles[0], doubles[1], fireMission.getTaskContent()+"地址");
        } else {
            //这里必须要写逻辑，不然如果手机没安装该应用，程序会闪退，这里可以实现下载安装该地图应用
            Toast.makeText(this, "尚未安装百度地图", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void openTencentMap() {
        ...
        if (MapUtil.isTencentMapInstalled()) {
            MapUtil.openTencentMap(this, 0, 0, null, doubles[0], doubles[1], fireMission.getTaskContent()+"地址");
        } else {
            //这里必须要写逻辑，不然如果手机没安装该应用，程序会闪退，这里可以实现下载安装该地图应用
            Toast.makeText(this, "尚未安装腾讯地图", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void openShareTencent() {
        double[] doubles = LatLngChangeNew.calWGS84toGCJ02(fireMission.getPosition().getLat(), fireMission.getPosition().getLng());
        WeChatShareUtils instance = WeChatShareUtils.getInstance(this);
        if(instance.isSupportWX()){
            String desc = "我分享了一个任务地址，赶快来看看吧。";
            String title = fireMission.getTaskContent()+"地址";
            String url = "http://api.map.baidu.com/marker?location="+doubles[0]+","+doubles[1]+"&title="+fireMission.getTaskContent()+"地址&output=html&src=webapp.baidu.openAPIdemo";
            ...
            Toast.makeText(this, "手机微信版本不支持分享功能", Toast.LENGTH_SHORT).show();
        }
    }
