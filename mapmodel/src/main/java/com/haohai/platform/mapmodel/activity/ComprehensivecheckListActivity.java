...
import com.haohai.platform.mapmodel.multitype.CompresiveListViewBinder;
...
public class ComprehensivecheckListActivity extends HhBaseActivity implements DatePicker.OnDateChangedListener,CheckPlanListStationViewBinder.OnOneBodyItemClick,CompresiveListViewBinder.OnPlanItemClick {
                    ...
                    public void call(Void aVoid) {
                        ...
                        if (fromMap) {
                            ...
                            if (shengLayout.getText().toString().equals("请选择省")) {
                                Toast.makeText(ComprehensivecheckListActivity.this, "请先选择省", Toast.LENGTH_SHORT).show();
                            } else {
                    ...
                    public void call(Void aVoid) {
                        ...
                        if (fromMap) {
                            ...
                            if (shiLayout.getText().toString().equals("请选择市")) {
                                Toast.makeText(ComprehensivecheckListActivity.this, "请先选择市", Toast.LENGTH_SHORT).show();
                            } else {
                                String currentshiId = "";
        ...
        RxViewAction.clickNoDouble(allText)
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        ...
                        gaojiStartimeText.setText("请开始时间");
                    }
                });
    ...
    private void getDataFromService() {
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "resource/api/plan/viewTheComprehensivePlan");
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        Toast.makeText(ComprehensivecheckListActivity.this, "数据获取失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private void initData() {
        ...
        if (checkplanList.size()==0){
            ...
            for (int i = 0; i < checkplanList.size(); i++) {
                ...
                if (checkplanList.get(i).getPlanResourceDTOS().size()>0) {
                    for (int j = 0; j < checkplanList.get(i).getPlanResourceDTOS().size(); j++) {
                        items.add(checkplanList.get(i).getPlanResourceDTOS().get(j));
                    }
                }
            }
        }
    ...
    private void register() {
        CompresiveListViewBinder compresiveListViewBinder = new CompresiveListViewBinder();
        compresiveListViewBinder.setListener(this);
        ...
        adapter.register(CheckPlanList.class, compresiveListViewBinder);
    ...
    public void onChecklistItemClickListener(CheckPlanList.PlanResourceDTOSFirejd planResourceDTOSFirejd,String type) {
        Log.e(TAG, "onChecklistItemClickListener: "+planResourceDTOSFirejd.getId() );
        if (type=="jiancha") {
        ...
        try {
            ...
            if (chooetime!=""){
                if (!gaojiStartimeText.getText().toString().equals("请输入开始时间")){
                    jsonObject.put("startTime", gaojiStartimeText.getText().toString()+" 00:00:00");
                }else {
                    jsonObject.put("startTime", chooetime + " 00:00:00");
                }
            }else {
                if (!gaojiStartimeText.getText().toString().equals("请输入开始时间")){
                    jsonObject.put("startTime", gaojiStartimeText.getText().toString()+" 00:00:00");
            ...
            if (chooseStatus!=0){
                jsonObject.put("status", chooseStatus);
            }
            if (!gaojiEndtimeText.getText().toString().equals("请输入结束时间")){
                jsonObject.put("endTime", gaojiEndtimeText.getText().toString()+" 00:00:00");
            }
            if (!quLayout.getText().toString().equals("请选择区")){
                jsonObject.put("gridName", quLayout.getText().toString());
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
