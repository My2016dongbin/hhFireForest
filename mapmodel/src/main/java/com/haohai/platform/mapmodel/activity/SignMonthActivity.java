...
import com.haohai.platform.mapmodel.bean.SignListModel;
...
import com.haohai.platform.mapmodel.model.SignMonth2;
...
//                                        mCalendarView.getCurYear(), mCalendarView.getCurMonth(), 23);
    ...
    private void postDataPost(String date) {
        SignListModel model = new SignListModel();
        model.setPage(1);
        model.setLimit(10);
        List<String> listS = new ArrayList<>();
        listS.add(new DbConfig(SignMonthActivity.this).getUser().getId());
        model.setDto(new SignListModel.Dto(listS,date));
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "api/attendance/record/month");//-2休息  -3旷工
        params.setBodyContent(new Gson().toJson(model));
        params.addHeader("Authorization", "bearer " + new DbConfig(SignMonthActivity.this).getUser().getToken());
        showDY3();
    ...
    private void postDataOld(String date) {
