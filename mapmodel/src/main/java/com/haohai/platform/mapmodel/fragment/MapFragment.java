...
//    private TextView gugeGaochengText;
    ...
    private View buildGridView(List<Grid> gridList) {
        ...
        for (int i = 0; i < gridList.size(); i++) {
            ...
            if(grid.getChildren()!=null && grid.getChildren().size()>0){
                //有子项 --> 递归
                ll_inOut.addView(buildGridView(grid.getChildren()));
            }else{
                //无子项 --> 类型选择
                ll_inOut.addView(buildGridTypeView(grid,resourceListList));
            }
            ll_out.addView(item);
        }
    ...
    private View buildGridTypeView(Grid grid, List<ResourceList> list) {
        View itemOut = LayoutInflater.from(getActivity()).inflate(R.layout.item_gridoutli,null);
        LinearLayout ll_out = itemOut.findViewById(R.id.ll_out);
