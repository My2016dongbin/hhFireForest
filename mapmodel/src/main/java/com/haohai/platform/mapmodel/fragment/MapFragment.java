...
//    private TextView gugeGaochengText;
            ...
            switch (what){
                ...
                case DIALOG_ONEBODY_FIRE_SHOW:
                    ...
                    for (int i = 0; i < oneBodyFireList.size(); i++) {
                        if (oneBodyFireList.get(i).getDto().getId().equals(oneBodyid)) {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        Log.e(TAG, "onActivityCreated: "+requestaddress.getRequstUrl() );
        //实例化IntentFilter对象
...
//启动导航组件
                        ...
                        LatLng latLng = new LatLngChange().transformFromWGSToGCJ(new LatLng(currentOneBodyFire.getDto().getAlarmLatitude(), currentOneBodyFire.getDto().getAlarmLongitude()));
                        ...
                        Poi end = new Poi(currentOneBodyFire.getDto().getName(), new com.amap.api.maps.model.LatLng(latLng.latitude, latLng.longitude), "");
        ...
        RxViewAction.clickNoDouble(yitijiOneView)
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        ...
                        picList.add(currentOneBodyFire.getDto().getPicPath1().replace("10.10.2.26","218.201.180.118"));
...
/**
        ...
        RxViewAction.clickNoDouble(yitijiTwoView)
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        ArrayList<String> picList = new ArrayList<>();
                        picList.add(currentOneBodyFire.getDto().getPicPath2().replace("10.10.2.26","218.201.180.118"));
                        ImagPagerUtil imagPagerUtil = new ImagPagerUtil(getActivity(), picList);
                        imagPagerUtil.setContentText("");
                        imagPagerUtil.show();
                    }
...
if (choose1 == 2){
                            ...
                            for (int j = 0; j < oneBodyFireList.size(); j++) {
                                if (oneBodyFireList.get(j).getDto().getIsReal()!=null) {
                                    if (oneBodyFireList.get(j).getDto().getIsReal()== 1) {
                                        oneBodyFireFenleiList.add(oneBodyFireList.get(j));
                                    }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        for (int i = 0; i < oneBodyFireList.size(); i++) {
                            if (oneBodyFireList.get(i).getDto().getId().equals(currentOneBodyFire.getDto().getId())) {
                                oneBodyFireList.get(i).getDto().setIsReal(isReleas);
                            }
                        }
                        initOneBodyFireModelData();
                        ...
                        for (int i = 0; i < oneBodyFireFenleiList.size(); i++) {
                            if (oneBodyFireFenleiList.get(i).getDto().getId().equals(currentOneBodyFire.getDto().getId())) {
                        ...
                        Message message = mHandler.obtainMessage();
                        Bundle b = new Bundle();
                        b.putString("id", oneBodyFireFenleiList.get(0).getDto().getId());
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        for (int i = 0; i < oneBodyFireAllList.size(); i++) {
                            if (oneBodyFireAllList.get(i).getDto().getIsReal()!=null) {
                                if (oneBodyFireAllList.get(i).getDto().getIsReal() == 1){
    ...
    private void initYitijiFireMap() {
        ...
        if (oneBodyFireFenleiList.size() > 0) {
            ...
            for (int i = 0; i < oneBodyFireFenleiList.size(); i++) {
                ...
                MapModel mapModel = new MapModel(oneBodyFire.getDto().getId(), oneBodyFire.getDto().getName(),new MapPosition(oneBodyFire.getDto().getAlarmLongitude(), oneBodyFire.getDto().getAlarmLatitude(),0.00), "ic_onebody");
...
/**
    ...
    private void initOneBodyFireModelData() {
        mingchengView.setText(currentOneBodyFire.getDto().getName());
        dizhiView.setText(currentOneBodyFire.getDto().getAddress());
        shijianView.setText(currentOneBodyFire.getDto().getAlarmDatetime().replace("T"," ").substring(0,currentOneBodyFire.getDto().getAlarmDatetime().indexOf(".")));
        jingweiduView.setText(currentOneBodyFire.getDto().getAlarmLongitude() +"、" + currentOneBodyFire.getDto().getAlarmLatitude());
        Glide.with(getContext()).load(currentOneBodyFire.getDto().getPicPath1().replace("10.10.2.26","218.201.180.118"))
                .error(R.drawable.ic_no_pic)
                .placeholder(R.drawable.ic_jaizai).into(yitijiOneView);
        Glide.with(getContext()).load(currentOneBodyFire.getDto().getPicPath2().replace("10.10.2.26","218.201.180.118"))
                .error(R.drawable.ic_no_pic)
                .placeholder(R.drawable.ic_jaizai).into(yitijiTwoView);

        if(currentOneBodyFire.getDto().getIsReal() == null){
            ...
            if (currentOneBodyFire.getDto().getIsReal() == 0){
...
/**
    ...
    private void initOneBodyFlyMap() {
        ...
        JSONObject posObj = new JSONObject();
        try {
            posObj.put("lat", currentOneBodyFire.getDto().getAlarmLatitude());
            posObj.put("lng", currentOneBodyFire.getDto().getAlarmLongitude());
            jsonObject.put("position", posObj);
        } catch (JSONException e) {
            e.printStackTrace();
        }
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        for(int i=0; i<data.length(); i++){
                            ...
                            if (dataListsObj.getInt("cameraType")==1){
                                kejianguangUrl= dataListsObj.getString("deviceId");
