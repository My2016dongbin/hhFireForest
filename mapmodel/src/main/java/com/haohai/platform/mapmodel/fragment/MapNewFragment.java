...
//    private TextView gugeGaochengText;
    ...
    public boolean isShowJiankong = false;
    public boolean isShowKk = false;
    public boolean isShowPark = false;
    public boolean isShowForest = false;
    ...
    private List<ResourceDTO> jiankongDTOList;
    private List<ResourceDTO> kkDTOList;
    private List<ResourceDTO> parkDTOList;
    private List<ResourceDTO> forestDTOList;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        jiankongDTOList = new ArrayList<>();
        kkDTOList = new ArrayList<>();
        parkDTOList = new ArrayList<>();
        forestDTOList = new ArrayList<>();
...
//                        gugeGaochengText.setTextColor(getResources().getColor(R.color.c7));
    ...
    private void initView() {
        ...
        flyBaiduMapZoom(36.085995,120.412886,14);//默认坐标
...
//        });
    ...
    private void initQuyuBianjie() {
        ...
        districtSearchOption.districtName("市南区");
...
/*
    ...
    private void initWeixinModelData() {
        ...
        fireTimeText.setText(parseTime(currentWeixingModel.getObservationDatetime())/*.substring(0, currentWeixingModel.getObservationDatetime().indexOf("."))*/);
        ...
        Log.e(TAG, "initWeixinModelData: ceshi4" + currentWeixingModel.getLightImageAddress());//112.6.182.92:18444
    ...
    private String parseTime(String time) {
        ...
        try{
            str = time.replace("T"," ").substring(0,19);
        }catch (Exception e){
            Log.e("",e.getMessage());
            return time;
        }
    ...
    public void onResourceListItemClickListener(ResourceList resourceList) {
        ...
        if (name.equals("视频监控点")){
            ...
            isShowLwt = isShow;
        }else if (name.equals("高点监控")){
            isShowJiankong = isShow;
        }else if (name.equals("出入卡口监控")){
            isShowKk = isShow;
        }else if (name.equals("公园区域监控")){
            isShowPark = isShow;
        }else if (name.equals("封山育林监控")){
            isShowForest = isShow;
    ...
    private void removeMarkerBaiduMap() {
        ...
        if (isShowJiankong){
            initResourceJiankongDataIntoBaiduMap();
        }
        if (isShowKk){
            initResourceKkDataIntoBaiduMap();
        }
        if (isShowPark){
            initResourceParkDataIntoBaiduMap();
        }
        if (isShowForest){
            initResourceForestDataIntoBaiduMap();
        }
        if (isShowZhihui){
    ...
    private void initResourceParkDataIntoBaiduMap() {
        Log.e(TAG, "initResourceDataIntoBaiduMap: " +parkDTOList.size() );
        List<OverlayOptions> options = new ArrayList<OverlayOptions>();
        for (int i = 0; i < parkDTOList.size(); i++) {
            BitmapDescriptor btm = BitmapDescriptorFactory.fromResource(R.drawable.kk);
            double[] doubles = LatLngChangeNew.calWGS84toBD09(parkDTOList.get(i).getPosition().getLat(), parkDTOList.get(i).getPosition().getLng());
            com.baidu.mapapi.model.LatLng point = new com.baidu.mapapi.model.LatLng(doubles[0], doubles[1]);
            Bundle bundle = new Bundle();
            bundle.putString("id", parkDTOList.get(i).getId());
            bundle.putInt("type", RESOURCE_OTHER);
            bundle.putString("address", parkDTOList.get(i).getAddress());
            bundle.putString("name", parkDTOList.get(i).getName());
            OverlayOptions option = new MarkerOptions()
                    .position(point)
                    .extraInfo(bundle)
                    .icon(btm);
    ...
    private void initResourceJiankongDataIntoBaiduMap() {
        Log.e(TAG, "initResourceDataIntoBaiduMap: " +jiankongDTOList.size() );
        List<OverlayOptions> options = new ArrayList<OverlayOptions>();
        for (int i = 0; i < jiankongDTOList.size(); i++) {
            BitmapDescriptor btm = BitmapDescriptorFactory.fromResource(R.drawable.kk);
            double[] doubles = LatLngChangeNew.calWGS84toBD09(jiankongDTOList.get(i).getPosition().getLat(), jiankongDTOList.get(i).getPosition().getLng());
            com.baidu.mapapi.model.LatLng point = new com.baidu.mapapi.model.LatLng(doubles[0], doubles[1]);
            Bundle bundle = new Bundle();
            bundle.putString("id", jiankongDTOList.get(i).getId());
            bundle.putInt("type", RESOURCE_OTHER);
            bundle.putString("address", jiankongDTOList.get(i).getAddress());
            bundle.putString("name", jiankongDTOList.get(i).getName());
            OverlayOptions option = new MarkerOptions()
                    .position(point)
                    .extraInfo(bundle)
                    .icon(btm);
    ...
    private void initResourceForestDataIntoBaiduMap() {
        Log.e(TAG, "initResourceDataIntoBaiduMap: " +forestDTOList.size() );
        List<OverlayOptions> options = new ArrayList<OverlayOptions>();
        for (int i = 0; i < forestDTOList.size(); i++) {
            BitmapDescriptor btm = BitmapDescriptorFactory.fromResource(R.drawable.kk);
            double[] doubles = LatLngChangeNew.calWGS84toBD09(forestDTOList.get(i).getPosition().getLat(), forestDTOList.get(i).getPosition().getLng());
            com.baidu.mapapi.model.LatLng point = new com.baidu.mapapi.model.LatLng(doubles[0], doubles[1]);
            Bundle bundle = new Bundle();
            bundle.putString("id", forestDTOList.get(i).getId());
            bundle.putInt("type", RESOURCE_OTHER);
            bundle.putString("address", forestDTOList.get(i).getAddress());
            bundle.putString("name", forestDTOList.get(i).getName());
            OverlayOptions option = new MarkerOptions()
                    .position(point)
                    .extraInfo(bundle)
                    .icon(btm);
    ...
    private void getResourceDataFromService(String ApiUrl, String nameresult) {
        ...
        } else if (nameresult.equals("高点监控")) {
            resorcetype = "jiankong";
        } else if (nameresult.equals("出入卡口监控")) {
            resorcetype = "kakou";
        } else if (nameresult.equals("公园区域监控")) {
            resorcetype = "park";
        } else if (nameresult.equals("封山育林监控")) {
            resorcetype = "forest";
        }
        JSONObject resorcelistobj = new JSONObject();
        try {
//            resorcelistobj.put("gridNoba",new DbConfig(getActivity()).getUser().getGridNo());
            resorcelistobj.put("gridNoba","370202");
        } catch (JSONException e) {
            e.printStackTrace();
        }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (resorcetype.equals("monitor")) { //视频监控
                        ...
                        Log.e(TAG, "onSuccess: " + teamDTOList );
                    }  else if (resorcetype.equals("jiankong")) {  //监控
                        Log.e(TAG, "onSuccess:jiankongDTOList " );
                        jiankongDTOList.clear();
                        jiankongDTOList = gson.fromJson(String.valueOf(data), new TypeToken<List<ResourceDTO>>() {
                        }.getType());
                        Log.e(TAG, "onSuccess: " + jiankongDTOList );
                    }  else if (resorcetype.equals("kakou")) {  //监控
                        Log.e(TAG, "onSuccess:kkDTOList " );
                        kkDTOList.clear();
                        kkDTOList = gson.fromJson(String.valueOf(data), new TypeToken<List<ResourceDTO>>() {
                        }.getType());
                        Log.e(TAG, "onSuccess: " + kkDTOList );
                    }  else if (resorcetype.equals("park")) {  //监控
                        Log.e(TAG, "onSuccess:parkDTOList " );
                        parkDTOList.clear();
                        parkDTOList = gson.fromJson(String.valueOf(data), new TypeToken<List<ResourceDTO>>() {
                        }.getType());
                        Log.e(TAG, "onSuccess: " + parkDTOList );
                    }  else if (resorcetype.equals("forest")) {  //监控
                        Log.e(TAG, "onSuccess:forestDTOList " );
                        forestDTOList.clear();
                        forestDTOList = gson.fromJson(String.valueOf(data), new TypeToken<List<ResourceDTO>>() {
                        }.getType());
                        Log.e(TAG, "onSuccess: " + forestDTOList );
    ...
    private void initResourceDataIntoBaiduMap() {
        if (resorcetype.equals("monitor")) {//视频监控点
            ...
            initResourceLwtDataIntoBaiduMap();
        }else if (resorcetype.equals("jiankong") ) {   //高点监控
            initResourceJiankongDataIntoBaiduMap();
        }else if (resorcetype.equals("kakou") ) {   //卡口
            initResourceKkDataIntoBaiduMap();
        }else if (resorcetype.equals("park") ) {   //公园
            initResourceParkDataIntoBaiduMap();
        }else if (resorcetype.equals("forest")) {   //育林区
            initResourceForestDataIntoBaiduMap();
