...
import com.ruyiruyi.rylibrary.utils.MapUtil;
import com.ruyiruyi.rylibrary.utils.NumberUtils;
import com.ruyiruyi.rylibrary.utils.WeChatShareUtils;
...
import static com.ruyiruyi.rylibrary.utils.WeChatShareUtils.weChatShareUtils;
...
//    private TextView gugeGaochengText;
    ...
    private Dialog guideChooseDialog;
    private View oneBodyFireInflater;
    private View guideChooseInflater;
    private TextView guide_local;
    private TextView guide_a_map;
    private TextView guide_bai_du;
    private TextView guide_tencent;
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        if (fireMissionList.size() > 0) {  //已经有数据
                            ...
                            if (!id.equals(fireMissionList.get(0).getId())) { //有新的报警数据
                                ...
                                Log.e(TAG, "onSuccess: 任务单" + fireMissionList.size());
                                Intent intent = new Intent(getContext(), BackgroundMp3Service.class);
                                intent.putExtra("type", "2"/*fireMissionList.get(0).getTaskType()*/);//2任务   12火警
...
//                        gugeGaochengText.setTextColor(getResources().getColor(R.color.c7));
   ...
   /*  //构建导航组件配置类，没有传入起点，所以起点默认为 “我的位置”
                ...
                .setSingleChoiceItems(items, choose1, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        if (choose1 == 2) {
                            ...
                            for (int j = 0; j < oneBodyFireList.size(); j++) {
                                if (oneBodyFireList.get(j).getIsReal() != null) {
                                    if (oneBodyFireList.get(j).getIsReal() == 1 && oneBodyFireList.get(j).getIsHandle() == 1) {
                                        oneBodyFireFenleiList.add(oneBodyFireList.get(j));
                                    }
                            ...
                            fenleiTextView.setText("未处理");
                            for (int j = 0; j < oneBodyFireList.size(); j++) {
                                if (oneBodyFireList.get(j).getIsReal() == null && oneBodyFireList.get(j).getIsHandle() != 1) {
                                    oneBodyFireFenleiList.add(oneBodyFireList.get(j));
                                }
                            }
                            //   currentPage = 1;
    ...
    private void initView() {
        ...
        flyBaiduMapZoom(27.863580,113.121054,10);//株洲
         ...
         * 导航选择Dialog
         **/
        guideChooseDialog = new Dialog(getContext(), R.style.ActionSheetDialogStyle);
        guideChooseInflater = LayoutInflater.from(getContext()).inflate(R.layout.dialog_guide_choose, null);
        guideChooseInflater.setMinimumWidth(10000);
        guide_local = ((TextView) guideChooseInflater.findViewById(R.id.guide_local));
        guide_a_map = ((TextView) guideChooseInflater.findViewById(R.id.guide_a_map));
        guide_bai_du = ((TextView) guideChooseInflater.findViewById(R.id.guide_bai_du));
        guide_tencent = ((TextView) guideChooseInflater.findViewById(R.id.guide_tencent));
        share_tencent = ((LinearLayout) guideChooseInflater.findViewById(R.id.share_tencent));
        dismiss = ((TextView) guideChooseInflater.findViewById(R.id.dismiss));
        dismiss.setOnClickListener(v -> {
            guideChooseDialog.dismiss();
        });
        guide_local.setOnClickListener(v -> {
            guideChooseDialog.dismiss();
            localGuide();
        });
        guide_a_map.setOnClickListener(v -> {
            guideChooseDialog.dismiss();
            openGaoDeMap();
        });
        guide_bai_du.setOnClickListener(v -> {
            guideChooseDialog.dismiss();
            openBaiDuMap();
        });
        guide_tencent.setOnClickListener(v -> {
            guideChooseDialog.dismiss();
            openTencentMap();
        });
        share_tencent.setOnClickListener(v -> {
            openShareTencent();
        });
        guideChooseDialog.setContentView(guideChooseInflater);
        Window guideChooseDialogWindow = guideChooseDialog.getWindow();
        guideChooseDialogWindow.setGravity(Gravity.BOTTOM);
        WindowManager.LayoutParams LpFire = guideChooseDialogWindow.getAttributes();
        guideChooseDialogWindow.setAttributes(LpFire);
        guideChooseDialog.setCanceledOnTouchOutside(true);
...
//        gugeGaochengText = ((TextView) mapChooseInflater.findViewById(R.id.gugegaocheng_text));
    ...
    private void openShareTencent() {
        ...
        if(instance.isSupportWX()){
            String desc = "我分享了一个火点，赶快来看看吧。";
            String title = currentOneBodyFire.getName();
            String url = "http://api.map.baidu.com/marker?location="+doubles[0]+","+doubles[1]+"&title="+currentOneBodyFire.getName()+"&output=html&src=webapp.baidu.openAPIdemo";
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                      ...
                      /*  //totalSize=data.getInt("totalSize");
                        ...
                        List<OneBodyFire.Dto> oneBodyFireAllList = gson.fromJson(String.valueOf(String.valueOf(data)), new TypeToken<List<OneBodyFire.Dto>>() {
                        }.getType());
                        //将所有疑似火情剔除
                        ...
                        if (choose1 == 1) {
                            ...
                            //已处理
                            for (int j = 0; j < oneBodyFireList.size(); j++) {
                                if (oneBodyFireList.get(j).getIsReal() != null && oneBodyFireList.get(j).getIsReal() == 1 && oneBodyFireList.get(j).getIsHandle() == 1) {
                                    oneBodyFireFenleiList.add(oneBodyFireList.get(j));
                                }
                            }
