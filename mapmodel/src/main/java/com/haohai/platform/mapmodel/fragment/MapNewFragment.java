...
import com.haohai.platform.firelibrary.ui.activity.FlameAddActivity;
import com.haohai.platform.firelibrary.ui.activity.FlameAreaActivity;
import com.haohai.platform.firelibrary.ui.activity.FlamePlanActivity;
...
import com.haohai.platform.mapmodel.activity.SettingActivity;
import com.haohai.platform.mapmodel.activity.SignMonthActivity;
import com.haohai.platform.mapmodel.activity.UploadActivity;
import com.haohai.platform.mapmodel.activity.UploadUserPostActivity;
...
public class MapNewFragment extends HhBaseFragment implements JsApi.OnJsClickListener, ResourceListViewBinder.OnResourceLsitItemClick, WeixingModelViewBinder.OnWeixingInfoItemClick, OneBodyFireViewBinder.OnOneBodyItemClick, DatePicker.OnDateChangedListener, INaviInfoCallback , SensorEventListener {
...
//    private TextView gugeGaochengText;
    ...
    private LinearLayout weixingShezhiView;
    private LinearLayout weixingChaxunView;
    private LinearLayout weixingLiebiaoView;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        timer = new Timer();
        /*timer.schedule(new TimerTask() {//循环请求
            @Override
            public void run() {
                // (1) 使用handler发送消息
    ...
    private void getGridName() {
        ...
        try {
            object.put("gridNo",new DbConfig(getActivity()).getUser().getGridNo());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL_LOGIN + "resource/api/grid/list");
        params.addHeader("Authorization", "bearer " + user.getToken());
        params.setBodyContent(object.toString());
        Log.e(TAG, "getGridName: " + params + object.toString() );
        x.http().post(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                Log.e(TAG, "onSuccess: getGridName" + result );
                if(Objects.equals(new DbConfig(getActivity()).getUser().getGridNo(), "370200")){
                    gridName = "青岛市";
                }
                ...
                try {
                    ...
                    JSONObject gridModel = (JSONObject) data.get(0);
                    String name = gridModel.getString("name");
                    String parentName = gridModel.getString("parentName");
                    if(name!=null && name.contains("区")){
                        isQu = true;
                        gridName = parentName;
                        quName = name;
                    }
                    if(name!=null && name.contains("市")){
                        isQu = false;
                        gridName = name;
                    }
                    if(name!=null && !name.contains("省")){
                        isQu = false;
                        gridName = name;
                    }
                    initQuyuBianjie();
            ...
            public void onError(Throwable ex, boolean isOnCallback) {
                Log.e(TAG, "onError: getGridName" + ex.toString() );
            }
    ...
    double[] toBaidu = gaoDeToBaidu(36.3032393109744, 120.30714139344022);
    int distance = 200;
    private final float zoom = 17;

    private void flySignCity() {
        flyPointBaidu(toBaidu[0],toBaidu[1]);
    }

    private void flyUser() {
        flyPointBaidu(CommonData.lat,CommonData.lng);
    }
    private void flyPointBaidu(double lat,double lng) {
        //飞到精确点上
    ...
    private void addCircle(com.baidu.mapapi.model.LatLng latLng) {
        /*if (null != mCircle) {
            mCircle.setCenter(latLng);
            return;
        }*/
        CircleOptions circleOptions = new CircleOptions();
        circleOptions.fillColor(0x666495ED); // 填充颜色
        circleOptions.stroke(new Stroke(3, 0xE66495ED));
        circleOptions.center(latLng);
        circleOptions.zIndex(1);
        circleOptions.radius(distance);
        mCircle = (Circle) mBaiduMap.addOverlay(circleOptions);
        Log.e(TAG, "addCircle: " );
    }
    ...
    private void bindView() {
        RxViewAction.clickNoDouble(fl_cu).subscribe(new Action1<Void>() {
            @Override
        ...
        RxViewAction.clickNoDouble(tv_setting).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
        ...
        RxViewAction.clickNoDouble(ll_setting).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
                startActivity(new Intent(getActivity(), SettingActivity.class));
            }
        });
        RxViewAction.clickNoDouble(ll_upload).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
                //startActivity(new Intent(getActivity(), UploadActivity.class));
                startActivity(new Intent(getActivity(), UploadUserPostActivity.class));
            }
        });
        RxViewAction.clickNoDouble(ll_flame).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
                startActivity(new Intent(getActivity(), FlamePlanActivity.class));
            }
        });
        RxViewAction.clickNoDouble(ll_line).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
                ARouter.getInstance().build(RouteUtils.History)
...
//                        imagPagerUtil.show();
         ...
         *  卫星火点附近焚烧区域
         */
        RxViewAction.clickNoDouble(fire_area)
                .subscribe(new Action1<Void>() {
                    @Override
                    public void call(Void aVoid) {
                        double[] doubles = LatLngChangeNew.calWGS84toBD09(Double.parseDouble(currentWeixingModel.getLatitude()),Double.parseDouble(currentWeixingModel.getLongitude()));
                        Intent intent = new Intent(getActivity(),FlameAreaActivity.class);
                        intent.putExtra("point","");
                        intent.putExtra("look",true);
                        intent.putExtra("latLng",doubles[0]+","+doubles[1]);
                        startActivity(intent);
                    }
                });
        /**
    ...
    private void signConfig() {
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "api/attendance/attConfigMessage");
        params.addHeader("Authorization", "bearer " + new DbConfig(getContext()).getUser().getToken());
        Log.e(TAG, "onSuccess: bingo params = " +  params.toString());
        x.http().post(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                Log.e(TAG, "onSuccess: bingo result = signConfig" +  result);
                mBaiduMap.clear();
                try {
                    JSONObject jsonObject = new JSONObject(result);
                    ...
                    if(array!=null && array.length()!=0){
                        for (int m = 0; m < array.length(); m++) {
                            JSONObject config = (JSONObject) array.get(m);
                            JSONObject positionLocation = config.getJSONObject("placePosition");
                            toBaidu[0] = positionLocation.getDouble("lat");
                            toBaidu[1] = positionLocation.getDouble("lng");
                            distance = Integer.parseInt(config.getString("deviation"));

                            addCircle(new com.baidu.mapapi.model.LatLng(toBaidu[0],toBaidu[1]));
                            //flySignCity();
                        }
                    }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if(data!=null && data.length()!=0){
                        //有记录
                        /*1 正常上班考勤 2 迟到上班考勤 3 出勤节点或签到时间未在规定节点中 6 正常下班考勤 7 早退下班考勤*/
                        for (int i = 0; i < data.length(); i++) {
                            ...
                            if(Objects.equals(obj.getString("signInState"), "1") || Objects.equals(obj.getString("signInState"), "2") || Objects.equals(obj.getString("signInState"), "3")){
                                if(Objects.equals(obj.getString("signInState"), "3") && signStatus == 1){
                                    signStatus = 2;
                                    break;
                                }else{
                                    signStatus = 1;
    ...
    private void sign(String causeStr) {
        ...
        workPosition.setLat(mCurrentLat);
        workPosition.setLng(mCurrentLon);
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "api/attendance/earlySign");
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if(cause.contains("范围外禁止打卡") || cause.equals("非正常考勤") ){
                        Toast.makeText(getActivity(), "范围外禁止打卡", Toast.LENGTH_SHORT).show();
    ...
    private void signOut(String causeStr) {
        ...
        Date curDate = new Date();
        signModel.setSignInTime(formatter.format(curDate));
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "api/attendance/nightSign");
        params.setBodyContent(new Gson().toJson(signModel));
        params.addHeader("Authorization", "bearer "  + new DbConfig(getContext()).getUser().getToken());
        Log.e(TAG, "onSuccess: bingo params = " +  params.toString());
        Log.e(TAG, "onSuccess: bingo new Gson().toJson(signModel) = " +  new Gson().toJson(signModel));
    ...
    private void showSendDialog(){
        ...
        normalDialog.setTitle(getResources().getString(R.string.app_name));
...
//        }
    ...
    private void initView() {
        ...
        ll_flame = ((LinearLayout) getView().findViewById(R.id.ll_flame));
        ll_line = ((LinearLayout) getView().findViewById(R.id.ll_line));
        tv_sign = ((TextView) getView().findViewById(R.id.tv_sign));
        tv_sign_out = ((TextView) getView().findViewById(R.id.tv_sign_out));
        tv_setting = ((TextView) getView().findViewById(R.id.tv_setting));
        ll_setting = ((LinearLayout) getView().findViewById(R.id.ll_setting));
        ll_upload = ((LinearLayout) getView().findViewById(R.id.ll_upload));
        ...
        fl_cu = ((FrameLayout) getView().findViewById(R.id.fl_cu));
        ...
        //设置最大最小缩放等级
        mBaiduMap.setMaxAndMinZoomLevel(zoom, 5);
        ...
        weixingShezhiView = ((LinearLayout) getView().findViewById(R.id.weixing_shezhi_view));
        weixingChaxunView = ((LinearLayout) getView().findViewById(R.id.weixing_chaxun_ciew));
        weixingLiebiaoView = ((LinearLayout) getView().findViewById(R.id.weixing_list_view));
        ...
        fire_area = ((TextView) weixingFireInflater.findViewById(R.id.fire_area));
    ...
    private void initQuyuBianjie() {
        ...
        districtSearchOption.cityName(gridName);//检索城市名称
        if(isQu){
            districtSearchOption.districtName(quName);//检索的区县名称
        }
        mDistrictSearch.searchDistrict(districtSearchOption);//请求行政区数据

        Log.e(TAG, "initQuyuBianjie: " + districtSearchOption.toString() );
    }
    ...
    private void getWeixingDataFromSetvice() {
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL_LOGIN + "fire/api/satelliteFirealarm/list");
...
//                        Toast.makeText(getContext(), jsonObject1.getString("message"), Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onError(Throwable ex, boolean isOnCallback) {
                Log.e(TAG, "onError: getWeixingDataFromSetvice " + ex.toString());
            }
