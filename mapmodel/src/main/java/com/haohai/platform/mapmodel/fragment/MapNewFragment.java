...
import com.haohai.platform.mapmodel.model.KakouDTO;
...
//    private TextView gugeGaochengText;
    ...
    private List<KakouDTO> kakouDTOList;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case DIALOG_MOINTOR_SHOW:
                    ...
                    getinfofromid(resourceid, false);
                    break;
                case TIME_CHANGE:
    ...
    public boolean isShowKakou = false;
    ...
    private void bindView() {
        ...
        mBaiduMap.setOnMarkerClickListener(new BaiduMap.OnMarkerClickListener() {
            ...
            public boolean onMarkerClick(Marker marker) {
                ...
                boolean isKakou = extraInfo.getBoolean("isKakou", false);
        ...
        RxViewAction.clickNoDouble(kejianguangbutton)
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        ...
                        intent.putExtra("deviceId", KkDeviceId);
                        intent.putExtra("serial", KkSerial);
...
//                        postIsReleaseFireToService();
    ...
    private void postIsReleaseFireToService() {
        ...
        if(isReleas == 1){//确认并下发
           ...
           params.addParameter("trueAlarmType",7);
        }
        if(isReleas == 3){//确认不下发
           params.addParameter("isAutoDelegate",2);
           params.addParameter("trueAlarmType",7);
        }
        if(isReleas == 0){//疑似火情
           params.addParameter("isAutoDelegate",2);
           params.addParameter("trueAlarmType",2);
        }
        Log.e(TAG, "getDataFromService: " + jsonObject.toString());
        params.addHeader("Authorization", "bearer " + new DbConfig(getContext()).getUser().getToken());
        params.addHeader("NetworkType", "Internet");
...
//        });
    ...
    private void initQuyuBianjie() {
        ...
        districtSearchOption.cityName("内江市");//检索城市名称 //TODO 1214

//        districtSearchOption.cityName("青岛市");//检索城市名称 //TODO 1214 五位一体
//        districtSearchOption.districtName("城阳区");//检索的区县名称 //TODO 1214 五位一体
...
//            jsonObject.put("startTime", before);//TODO bingo
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        //Toast.makeText(getContext(), "数据获取失败", Toast.LENGTH_SHORT).show(); TODO
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
/*        Glide.with(getContext()).load(currentOneBodyFire.getPicPath2())
    ...
    public void onResourceListItemClickListener(ResourceList resourceList) {
        for (int i = 0; i < resourceListList.size(); i++) {
            if (resourceListList.get(i).getId().equals(resourceList.getId())) {
                ...
                if (resourceList.getName().equals("视频监控点") || resourceList.getName().equals("道闸") || resourceList.getName().equals("卡口监控点") || resourceList.getName().contains("水源地") || resourceList.getName().equals("墓地")
                        || resourceList.getName().equals("护林检查站") || resourceList.getName().equals("消防专业队") || resourceList.getName().equals("危险源") || resourceList.getName().equals("物资储备库") || resourceList.getName().equals("瞭望塔") || resourceList.getName().equals("森林防火监测中心")) {
    ...
    private void removeMarkerBaiduMap() {
        ...
        if (isShowKakou) {
            initResourceKakouDataIntoBaiduMap();
        }
        if (isShowDaozha) {
    ...
    private void initResourceKakouDataIntoBaiduMap() {
        List<OverlayOptions> options = new ArrayList<OverlayOptions>();
        for (int i = 0; i < kakouDTOList.size(); i++) {
            Matrix matrix = new Matrix();
            Bitmap bitmap = ((BitmapDrawable) getResources().getDrawable(R.drawable.ic_monitor_shashi)).getBitmap();
            bitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
            BitmapDescriptor marker = BitmapDescriptorFactory.fromBitmap(bitmap);
            double[] doubles = LatLngChangeNew.calWGS84toBD09(kakouDTOList.get(i).getPosition().getLat(), kakouDTOList.get(i).getPosition().getLng());
            com.baidu.mapapi.model.LatLng point = new com.baidu.mapapi.model.LatLng(doubles[0], doubles[1]);
            Bundle bundle = new Bundle();
            bundle.putString("id", kakouDTOList.get(i).getId());
            bundle.putInt("type", RESOURCE_MONITOR);
            bundle.putBoolean("isKakou", true);
            OverlayOptions option = new MarkerOptions()
                    .position(point)
                    .extraInfo(bundle)
                    .icon(marker);
    ...
    private void initResourceDaozhaDataIntoBaiduMap() {
        ...
        for (int i = 0; i < daozhaDTOList.size(); i++) {
            ...
            bitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
            BitmapDescriptor marker = BitmapDescriptorFactory.fromBitmap(bitmap);
            double[] doubles = LatLngChangeNew.calWGS84toBD09(daozhaDTOList.get(i).getPosition().getLat(), daozhaDTOList.get(i).getPosition().getLng());
            com.baidu.mapapi.model.LatLng point = new com.baidu.mapapi.model.LatLng(doubles[0], doubles[1]);
            Bundle bundle = new Bundle();
    ...
    private void getResourceDataFromService(String ApiUrl, String nameresult) {
        ...
        Log.e(TAG, "getResourceDataFromService: " + "resource" + ApiUrl + "/list");
        params.setAsJsonContent(true);
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (resorcetype.equals("monitor")) { //视频监控
                        ...
                        kakouDTOList.clear();
                        kakouDTOList = gson.fromJson(String.valueOf(data), new TypeToken<List<KakouDTO>>() {
                        }.getType());
    ...
    private void initResourceDataIntoBaiduMap() {
        if (resorcetype.equals("monitor")) {
            initResourceMonitoerDataIntoBaiduMap();
        } else if (resorcetype.equals("kakou")) {
            initResourceKakouDataIntoBaiduMap();
     ...
     * 根据监控点id获取信息
     */
    private void getinfofromid(final String resourceid, boolean isKakou) {
        showDialogProgress(progressDialog, "加载中...");
        //获取摄像头URL
        JSONObject jsonObject = new JSONObject();
        try {
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                Log.e(TAG, "onSuccess: --1- ing" + result);
                try {
                    JSONObject jsonObject1 = new JSONObject(result);
                    if (jsonObject1.getString("code").equals("200")) {
                        JSONArray data = jsonObject1.getJSONArray("data");
                        ...
                        for (int i = 0; i < data.length(); i++) {
                            ...
                            if (i == 0 && dataListsObj.getString("name").contains("球")) {
                                kejianguangUrl = dataListsObj.getString("id");
                                kejianguangMId = dataListsObj.getString("monitorId");
                                KkDeviceId = dataListsObj.getString("deviceId");
                                KkSerial = dataListsObj.getString("serial");
                                break;
                            }
                        ...
                        if (isKakou) {
                            ...
                            kejianguangMId = data.getJSONObject(0).getString("monitorId");
                            kejianguangUrl = data.getJSONObject(0).getString("id");
                            KkDeviceId = data.getJSONObject(0).getString("deviceId");
                            KkSerial = data.getJSONObject(0).getString("serial");
                        } else {
                        ...
                        //获取详情
                        getShipinReourceXiangqing(resourceid, isKakou);
                    } else {
                        Toast.makeText(getContext(), "数据获取失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
    ...
    private void getShipinReourceXiangqing(String resourceid, boolean isKakou) {
        ...
        infoparams.addHeader("NetworkType", "Internet");
