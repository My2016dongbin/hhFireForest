...
public class LoginActivity extends HhBaseActivity {
    ...
    private void getUserInfo() {
        ...
        final RequestParams params = new RequestParams(RequestUtils.LOGIN_URL + "auth/api/auth/user/get/app/userinfo");
        // params.addBodyParameter("reqJson", jsonObject.toString());
        params.setConnectTimeout(10000);
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        ...
                        String gridName = userJsonObj.getString("gridName");
                        ...
                        String attendanceDate = userJsonObj.getString("attendanceDate");
                        if(attendanceDate!=null && !attendanceDate.isEmpty() && !Objects.equals(attendanceDate, "null")){
                            CommonData.workDayList = Arrays.asList(attendanceDate.split(","));
                        }else{
                            attendanceDate = "1,2,3,4,5,6,7";
                            CommonData.workDayList = Arrays.asList(attendanceDate.split(","));
                        }
                        String earlyWorkTime = userJsonObj.getString("earlyWorkTime");
                        String nightWorkTime = userJsonObj.getString("nightWorkTime");
                        @SuppressLint("SimpleDateFormat") SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                        if(earlyWorkTime!=null && !Objects.equals(earlyWorkTime, "null") && !earlyWorkTime.isEmpty()){
                           try {
                                Date parse = simpleDateFormat.parse("2024-02-01 " + earlyWorkTime + ":00");
                                CommonData.earlyHour = parse.getHours();
                                CommonData.earlyMinute = parse.getMinutes();
                           } catch (ParseException e) {
                                e.printStackTrace();
                            }
                        }
                        if(nightWorkTime!=null && !Objects.equals(nightWorkTime, "null") && !nightWorkTime.isEmpty()){
                            try {
                                Date parse = simpleDateFormat.parse("2024-02-01 " + nightWorkTime + ":00");
                                CommonData.nightHour = parse.getHours();
                                CommonData.nightMinute = parse.getMinutes();
                            } catch (ParseException e) {
                                e.printStackTrace();
                            }
                        }
                        ...
                        User user = new User(id, userCode,userNameEdit.getText().toString(), passwordEdit.getText().toString(), fullName, email, phone, sex, entryTime, birthday, type, isSuperAdmin, comment, groupId,
                                gridNo,gridName, bkchar2, money, lockMoney, groupName, state, 1, access_token,headUrl);
                        user.isShangchuan = false;
                        user.setIsyunyin(0);
                        user.setHasMainMap(true);
                        user.setHasMainVideo(true);
                        user.setHasMainApp(true);
                        user.setHasMainMy(true);
                        user.setEarlyHour(CommonData.earlyHour);
                        user.setEarlyMinute(CommonData.earlyMinute);
                        user.setNightHour(CommonData.nightHour);
                        user.setNightMinute(CommonData.nightMinute);
                        user.setWorkDayList(CommonData.workDayList.toString().replace("[","").replace("]",""));
                        ...
                        tagSet.add(gridNo);
                        tagSet.add("fushan3_" + gridNo);
                        tagSet.add("fushan3_" + groupId);
                        tagSet.add(id);
                        Log.e(TAG, "tagSet: "+gridNo + "，" + id);
