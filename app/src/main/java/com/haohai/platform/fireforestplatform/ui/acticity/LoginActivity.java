...
public class LoginActivity extends HhBaseActivity {
   ...
   /* private void doLogin() {
    ...
    private void postPer(String id) {
        RequestParams params = new RequestParams("http://192.168.1.12:8012/" + "auth/api/auth/auth/list/element/from/menu");
    ...
    private void postMainPer() {
        RequestParams params = new RequestParams("http://192.168.1.12:8012/" + "auth/api/auth/auth/user/auth");
        params.addHeader("Authorization", "bearer " + new DbConfig(this).getUser().getToken());
        Log.e(TAG, "postPermissions: " + params);
        x.http().get(params, new Callback.CommonCallback<String>() {
            @Override
